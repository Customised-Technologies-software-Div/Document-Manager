...
namespace DocumentManager.com.Utility
{
    public class ReportProcessor
    {
        ...
        public void SetValue(ICell cell, string fieldName, Field field)
        {
            ...
            //if(fieldName.Equals("Invoice No"))
            //{
            //    OleDbConnection conn = DatabaseConnection.GetConnection();
            //    conn.Open();
            //    string sqlString = "select svalue from settings where setting=@1";
            //    OleDbCommand cmd = new OleDbCommand(sqlString,conn);
            //    cmd.Parameters.AddWithValue("@1", "ServiceInvoice");
            //    OleDbDataReader dbReader = cmd.ExecuteReader();
            //    if (dbReader.Read())
            //    {
            //        cell.SetCellValue(dbReader["svalue"].ToString());
            //    }
            //}
        }
    }
